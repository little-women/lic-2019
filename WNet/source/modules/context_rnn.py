...
class ExternalKnowledge(nn.Module):
    ...
    def __init__(self, vocab, embedding_dim, hop, dropout, padding_idx,
                 ablationH=False, ablationG=False):
        ...
        self.dropout_layer = nn.Dropout(dropout)
        self.padding_idx = padding_idx
        self.ablationH = ablationH
        self.ablationG = ablationG
    ...
    def add_lm_embedding(self, full_memory, kb_len, conv_len, hiddens):
        for bi in range(full_memory.size(0)):
            ...
            full_memory[bi, start:end, :] = full_memory[
                bi, start:end, :] + hiddens[bi, :conv_len[bi], :]
        return full_memory
    ...
    def load_memory(self, story, kb_len, conv_len, hidden, dh_outputs):
        ...
        for hop in range(self.max_hops):
            ...
            if not self.ablationH:
                embed_A = self.add_lm_embedding(
                    embed_A, kb_len, conv_len, dh_outputs)
            ...
            embed_C = self.C[
                hop + 1](story.contiguous().view(story_size[0], -1).long())
            embed_C = embed_C.view(story_size + (embed_C.size(-1),))
            embed_C = torch.sum(embed_C, 2).squeeze(2)
            if not self.ablationH:
                embed_C = self.add_lm_embedding(
                    embed_C, kb_len, conv_len, dh_outputs)
...
class LocalMemoryDecoder(nn.Module):
    ...
    def forward(self, extKnow, story_size, story_lengths, copy_list,
        ...
        all_decoder_outputs_vocab = _cuda(torch.zeros(
            max_target_length, batch_size, self.num_vocab))
        all_decoder_outputs_ptr = _cuda(torch.zeros(
            max_target_length, batch_size, story_size[1]))
            ...
            if get_decoded_words:
                ...
                for bi in range(batch_size):
                    ...
                    if '@' in self.lang.index2word[token]:
                        ...
                        if args['record']:
                            memory_mask_for_step[
                                bi, toppi[:, i][bi].item()] = 0
                    else:
